...
def isValidWord(word, hand, wordList):
    ...
    for letters_word in word_frequency:
        if letters_word in hand.keys():
            if word_frequency[letters_word] <= hand[letters_word]:
                pass
            else:
                return False
        else:
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        if user_word == ".":
            ...
            # Otherwise (the word is valid):
                ...
                word_score = getWordScore(user_word,HAND_SIZE)
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
    ...
    '''else:
            if user_word in wordList:
                score = int(getWordScore(user_word, calculateHandlen(hand)))
                hand = updateHand(hand, user_word)
                total_score += score
...
def playGame(wordList):
    ...
    while True:
        ...
        if word == 'n':
            hand = dealHand(HAND_SIZE)
            #new_hand_size = calculateHandlen(hand)
